...
//        loggDiff(expectedXml, edited)
    ...
    fun verifyRequestAsXml_newWay() {
        ...
        // Verify that it works OK also with prolog
        val completePayload = """<?xml version="1.0" encoding="UTF-8"?>""" + payloadFromExpectedXmlFile
        val doc = builder.buildFellesformatDocument(fellesformat.mottakenhetBlokk, completePayload.toByteArray())
        val xml = builder.toXml(doc)
        // Verify that we get expected XML (remove whitespace)
